...
$BODY$
 ...
 BEGIN	
	...
	SELECT padrao_sistema, isento INTO v_padrao_sistema, v_isenta FROM scr_tipo_imposto WHERE scr_tipo_imposto.id_tipo_imposto = v_tipo_imposto;
	-- Nao calcula se tipo de imposto for isento
	IF v_isenta = 0 THEN 		

		IF v_padrao_sistema = 0 THEN
	...
	WHERE
		calculo_difal = 0 OR v_tipo_imposto IN (2,11)
	UNION
	SELECT 
		NULL::integer as id_cf,
		NULL::integer as id,
	...
	WHERE
		...
		calculo_difal = 1 
		AND v_tipo_imposto NOT IN (2,11)
	UNION
	SELECT 
		NULL::integer as id_cf,
		NULL::integer as id,
	...
	IF calculo_difal = 1 AND v_tipo_imposto <> 11 THEN 		
	...
	IF 
		imposto_incluso = 2 
		AND (
			(v_tipo_imposto NOT IN (6,7,8) )
			OR (v_tipo_imposto = 9 AND LEFT(remetente_cnpj,8) <> '33337122')  --Caso Especial (Generalizar em regra)
		)
	THEN
