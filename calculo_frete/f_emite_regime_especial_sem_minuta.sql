-- Function: public.f_emite_regime_especial_sem_minuta(integer[], date, integer, integer, refcursor, refcursor, refcursor, refcursor, refcursor)

-- DROP FUNCTION public.f_emite_regime_especial_sem_minuta(integer[], date, integer, integer, refcursor, refcursor, refcursor, refcursor, refcursor);

CREATE OR REPLACE FUNCTION public.f_emite_regime_especial_sem_minuta(
    lstnf integer[],
    p_data_cte_re date,
...
DECLARE
	...
	--Parametros Para Receber Calculo do Imposto
	v_id_aux			integer;
...
BEGIN		
	...
	IF p_id_cte_re = 0 THEN 
		WITH t AS (
			SELECT
				id,
				CASE WHEN vTipoDocumento = 1 THEN tipo_imposto ELSE 2 END as tipo_imposto,
				aliquota,
				aliquota_icms_st,
				calcula_difal,
				aliq_icms_inter,
		...
		UPDATE scr_notas_fiscais_imp SET
			...
			data_cte_re = p_data_cte_re --Primeira Nota vai determinar a data do conhecimento
		FROM
			t
		WHERE
	...
	FOREACH nf IN ARRAY lstnf LOOP
		...
		IF vTipoTransporte = 1 THEN 
			v_id_conhecimento = f_lanca_nota_fiscal_conhecimento(nf,v_id_conhecimento,4);
		ELSE
	...
	----------------------------------------------------------------------------------------------------------
	IF p_id_cte_re <> 0 THEN
		DELETE FROM scr_conhecimento_cf WHERE id_conhecimento = p_id_cte_re;
	END IF;

	RAISE NOTICE 'parametros: %', f_scr_parametros_calculo_frete(v_id_conhecimento);
	PERFORM f_scr_calcula_frete(f_scr_parametros_calculo_frete(v_id_conhecimento),'cf','msg');
...
----------------------------------------------------------------------------------------------------------
	...
	UPDATE 	scr_conhecimento SET
			...
			peso 			= tbl.total_peso,
			numero_ctrc_filial 	= COALESCE(numero_ctrc_filial, vNumeroConhecimento),
			numero_documento 	= COALESCE(numero_documento, vNumeroDocumento),			
