...
SELECT --DISTINCT ON (nf.id_nota_fiscal_imp)
	...
	--Coluna de Compatilidade, Regime Especial de SP, 
	--agora extendido a todos Estados como CTe Global
	CASE
		WHEN 	r.regime_especial_mg = 1 
			AND co.uf = cd.uf 
			AND co.uf <> 'MG'
			AND nf.limite_minimo_re = 1 
		THEN 
			1
		ELSE 
			0
   ...
   FROM scr_notas_fiscais_imp nf
     ...
     LEFT JOIN v_regime_especial_combinado conf_re 
		ON conf_re.codigo_cliente = r.codigo_cliente
     LEFT JOIN cliente_enderecos re 
		ON re.codigo_cliente = r.codigo_cliente AND re.id_tipo_endereco = 3
     LEFT JOIN cidades cr 
		ON cr.id_cidade = re.id_cidade::integer
     LEFT JOIN cliente d 
		ON d.codigo_cliente = nf.destinatario_id
     LEFT JOIN cliente_enderecos de 
		ON de.codigo_cliente = d.codigo_cliente AND de.id_tipo_endereco = 3
     LEFT JOIN cidades cd 
		ON cd.id_cidade = de.id_cidade::integer
     LEFT JOIN cliente p 
		ON p.codigo_cliente = nf.consignatario_id
     LEFT JOIN cliente_enderecos pe 
		ON pe.codigo_cliente = p.codigo_cliente AND pe.id_tipo_endereco = 3
     LEFT JOIN cidades cp 
		ON pe.id_cidade::integer = cp.id_cidade
     LEFT JOIN fornecedores m 
		ON m.id_fornecedor = nf.id_motorista
     LEFT JOIN v_transportadoras t 
		ON nf.redespachador_id = t.id_fornecedor
     LEFT JOIN cidades ct 
		ON t.id_cidade::integer = ct.id_cidade
     LEFT JOIN scr_conhecimento c 
		ON c.id_conhecimento = nf.id_conhecimento
     LEFT JOIN cidades co 
		ON co.id_cidade = nf.calculado_de_id_cidade
     LEFT JOIN cidades cde 
		ON cde.id_cidade = nf.calculado_ate_id_cidade
     LEFT JOIN estado uf_dest 
		ON cde.uf = uf_dest.id_estado
     LEFT JOIN filial 
		ON filial.codigo_empresa = nf.empresa_emitente 
			AND filial.codigo_filial = nf.filial_emitente
     LEFT JOIN cidades filial_cidade 
		ON filial_cidade.id_cidade = filial.id_cidade::integer
     LEFT JOIN filial filial2 
		ON filial2.codigo_empresa = p.empresa_responsavel 
			AND filial2.codigo_filial = '001'::bpchar
     LEFT JOIN cidades filial2_cidade 
		ON filial2_cidade.id_cidade = filial2.id_cidade::integer
     LEFT JOIN scr_romaneios rom 
		ON rom.id_romaneio = nf.id_romaneio
     LEFT JOIN scr_ocorrencia_edi o 
		ON o.codigo_edi = nf.id_ocorrencia
     LEFT JOIN scr_tabelas_tipo_veiculo ttv 
		ON ttv.id_tipo_veiculo = nf.id_tipo_veiculo
     ...
     LEFT JOIN veiculos veic
		ON veic.placa_veiculo = nf.placa_veiculo  
     LEFT JOIN v_regiao_setores v_rs 
		ON CASE WHEN v_rs.tipo_composicao = 1 THEN current_database() NOT IN ('softlog_medilog','softlog_transribeiro') AND v_rs.id_cidade = d.id_cidade::integer
		        WHEN v_rs.tipo_composicao = 2 THEN current_database() NOT IN ('softlog_medilog','softlog_transribeiro') AND v_rs.id_bairro = d.id_bairro::integer
		        ELSE false
		   END;
