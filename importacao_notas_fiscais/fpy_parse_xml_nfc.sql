CREATE OR REPLACE FUNCTION public.fpy_parse_xml_nfc(xml text)
  RETURNS json AS
$BODY$
import xmltodict
import json 
...
n['nfe_dest_cnpj_cpf'] = '00000000000'
n['nfe_dest_cod_mun']  = emit_cod_mun
...
n['nfe_valor_desc'] = onf.get('vDesc')
...
produtos = xml_nfe['nfeProc']['NFe']['infNFe']['det']
if type(produtos) == type(OrderedDict()):
    produtos = [produtos]
...
for item in produtos:
    ...
    pr['produto_ncm'] = item['prod']['NCM']
    pr['produto_cfop'] = item['prod']['CFOP']
    pr['produto_unidade'] = item['prod']['uCom']
    pr['produto_qtd'] = item['prod']['qCom']
    pr['produto_vl_unit'] = item['prod']['vUnCom']
...
ALTER FUNCTION fpy_parse_xml_nfc(xml text)  OWNER TO softlog_centroeste;