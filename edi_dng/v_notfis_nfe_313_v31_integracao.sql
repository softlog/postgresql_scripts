...
CREATE OR REPLACE VIEW public.v_notfis_nfe_313_softlog_v31 AS 
 SELECT rnf.id_nota_fiscal_imp AS id_registro,
	...
	|| btrim(to_char(c.total_frete, '0000000000000V99'::text))
	|| lpad(COALESCE(nf.id_nota_fiscal_imp::text, '0'::text), 15, '0')
	|| lpad(COALESCE(rnf.id_romaneio::text, '0'::text), 15,'0' )
	|| lpad(COALESCE(0::text, '0'::text), 15,'0' )
	|| lpad(COALESCE(c.id_conhecimento::text, '0'::text), 15,'0' )
	|| lpad('400'::text, 3, '0') 
	|| lpad(COALESCE(conex.id_string_conexao::text, '0'::text), 15,'0')
   ...
   FROM scr_notas_fiscais_imp nf
	...
	LEFT JOIN string_conexoes conex
		ON conex.banco_dados = current_database()
...
SELECT nf.id_conhecimento_notas_fiscais AS id_registro,
	...
	|| btrim(to_char(c.total_frete, '0000000000000V99'::text))
	|| lpad(COALESCE(nf.id_nota_fiscal_imp::text, '0'::text), 15, '0')
	|| lpad(COALESCE(rnf.id_romaneios::text, '0'::text), 15,'0' )
	|| lpad(COALESCE(nf.id_conhecimento_notas_fiscais::text, '0'::text), 15,'0' )
	|| lpad(COALESCE(c.id_conhecimento::text, '0'::text), 15,'0' )
	|| lpad('401'::text, 3, '0') 
	|| lpad(COALESCE(conex.id_string_conexao::text, '0'::text), 15,'0')
	|| chr(13)|| chr(10)  AS registro
...
SELECT * FROM f_edi_notfis_nfe_romaneio_softlog(31243) as texto_edi;
SELECT * FROM f_edi_notfis_nfe_romaneio(31243) as texto_edi;

-- SELECT * FROM v_notfis_nfe_313_softlog_v31 WHERE id_romaneio = 31243
...
SELECT 
	...
	SELECT chave_nfe, chave_cte, id_romaneio, id_nota_fiscal_imp, id_conhecimento  FROM v_mgr_notas_fiscais WHERE chave_nfe = '53210406234797000178550010011697391100275910'

	SELECT numero_romaneio FROM scr_romaneios WHERE id_romaneio = 31243	
	WITH t AS (
		SELECT 			
	...
	FROM 
		t;
SELECT * FROM v_usuarios ORDER BY 1
WITH t AS (
		SELECT
			r311.registro || 
			r312.registro || 
