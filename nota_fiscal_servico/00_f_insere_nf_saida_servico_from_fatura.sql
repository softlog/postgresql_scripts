...
UPDATE com_nf SET emite_nfse = 1 WHERE id_nf = 24
...
UPDATE scr_faturamento SET gera_nfse = 0 WHERE id_faturamento = 3564;
UPDATE scr_faturamento SET gera_nfse = 1 WHERE id_faturamento = 3564;
SELECT * FROM nfse_tmp

DELETE FROM com_nf;
DELETE FROM com_nf_itens
SELECT * FROM com_nf
SELECT * FROM com_nf_itens;
...
CREATE OR REPLACE FUNCTION public.f_tgg_insere_nf_servico_from_fatura()
  RETURNS trigger AS
$BODY$
DECLARE
...
------------------------------------------------------------------------------------------------------------------	
	...
	IF OLD.gera_nfse = 1 AND NEW.gera_nfse = 1 THEN 
		RAISE NOTICE 'Saiu';
		RETURN NEW;
	END IF;

	IF NEW.gera_nfse = 0 THEN 
		RETURN NEW;
	END IF;
	...
	v_dest_cnpj_cpf		= NEW.fatura_cnpj;

	SELECT 
		cidades.cod_ibge, cliente.codigo_cliente, COALESCE(cliente_parametros.valor_parametro::integer,0)
	INTO 
		v_dest_cod_mun, v_dest_codigo, v_gera_nfse
	FROM 
		cliente
		LEFT JOIN cidades
			ON cidades.id_cidade = cliente.id_cidade
		LEFT JOIN cliente_parametros
			ON cliente_parametros.codigo_cliente = cliente.codigo_cliente AND id_tipo_parametro = 152
	WHERE 
		cliente.codigo_cliente = NEW.fatura_sacado_id;


	IF v_gera_nfse = 0 THEN
		RETURN NEW;
	END IF;
	...
	SELECT 
		cnpj, cidades.cod_ibge, filial.id_cidade
	INTO 
		v_emit_cnpj_cpf, v_emit_cod_mun, v_id_cidade_nfse
	FROM 
		filial
		LEFT JOIN cidades 
			ON cidades.id_cidade = filial.id_cidade		
	WHERE 
		codigo_empresa = NEW.codigo_empresa 
		AND codigo_filial = NEW.codigo_filial;
...
-----------------------------------------------------------------------------------------------------------------
	...
	v_servico_cod_mun = v_emit_cod_mun;	
	...
	SELECT serie, codigo_servico, filial_parametros_iss.aliquota_iss, natureza_tributacao, tipo_tributacao, exigibilidade 
	INTO v_serie, v_natureza_operacao, v_aliquota, v_natureza_tributacao, v_tipo_tributacao, v_exigibilidade
	FROM filial_parametros_iss
		LEFT JOIN filial 
			ON filial.id_filial = filial_parametros_iss.id_filial				
	WHERE 
		filial.codigo_filial = NEW.codigo_filial AND 
		filial.codigo_empresa = NEW.codigo_empresa AND
		filial_parametros_iss.id_cidade = v_id_cidade_nfse;


	/*	
	RAISE NOTICE 'Aliquota %', v_aliquota;
	RAISE NOTICE 'ORIGEM %', v_id_cidade_nfse;
	...
	*/
	...
	WITH t AS (
		...
		WHERE 
			--id_faturamento = 3564
			id_faturamento = NEW.id_faturamento
	...
	, t1 AS (
		...
		SELECT 		
			...
			SUM(valor_servico) as valor_servico_total
		FROM
			t
	...
	v_observacao = 'NFSe referente a Fatura ' || NEW.numero_fatura || ': ' || v_observacao;
	...
	BEGIN 
		...
		INSERT INTO com_nf(
			...
			v_numero_doc,--8
			0,--9
			NEW.codigo_empresa,--10
			NEW.codigo_filial,--11
			v_dest_cnpj_cpf,--12
			NULL,--13
			NULL,--14
			v_modelo,--15
