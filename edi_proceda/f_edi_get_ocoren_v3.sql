...
BEGIN
	...
	SELECT 	
		...
		COALESCE(oc.codigo_edi, o.id_ocorrencia_proceda, o.codigo_edi,0) as cod_ocorrencia,
		...
		CASE 
			...
			WHEN COALESCE(o.codigo_edi,0) IN (1,2)	THEN ''ENTREGA REALIZADA. RECEBIDA POR: '' || 
		...
		LEFT(COALESCE(c.nome_recebedor,''''),20)::text as nome_recebedor,
		COALESCE(c.cpf_recebedor,'''')::text as doc_recebedor,
		''''::text as observacao_35
	FROM 	
		hora,
		...
		LEFT JOIN cliente_edi_ocoren_depara oc
			ON oc.codigo_cliente = c.remetente_id
	...
	SELECT 	
		...
		nf.chave_nfe,
		trim(nf.numero_pedido_nf) as numero_pedido_nf,
		LEFT(COALESCE(nf.nome_recebedor,''''),20)::text as nome_recebedor,
		COALESCE(nf.cpf_recebedor,'''')::text as doc_recebedor,
		''''::text as observacao_35		
	FROM 	
		hora,
				...
				SELECT 	
					...
					CASE 
						WHEN t.cod_ocorrencia = 0 AND current_database() = ''softlog_assislog'' THEN 
							''0200108'' ||  t.chave_nfe
						ELSE 
							COALESCE(replace(t.observacao_livre,chr(13)||chr(10),''''),'''') 
					END::text as observacao_livre,
					left(COALESCE(replace(t.observacao_livre,chr(13)||chr(10),''''),''''),70) as observacao_livre_2,
					t.espaco,
