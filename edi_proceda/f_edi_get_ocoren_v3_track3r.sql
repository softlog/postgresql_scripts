-- Function: public.f_edi_get_ocoren_v3()
-- DROP FUNCTION public.f_edi_get_ocoren_v3(integer[]);
CREATE OR REPLACE FUNCTION public.f_edi_get_ocoren_v3_track3r(lst_conhecimentos integer[])
  RETURNS json AS
$BODY$
DECLARE
...
BEGIN
	...
	SELECT 	
		...
		o.id_ocorrencia as cod_ocorrencia,
	...
	FROM 	
		...
		LEFT JOIN scr_ocorrencias_track3r o
			ON o.id_ocorrencia_softlog = nf.id_ocorrencia
	...
	WHERE 
		...
		o.id_ocorrencia IS NOT NULL AND 
		CASE 
	...
	FROM 	
		...
		LEFT JOIN scr_ocorrencias_track3r o
			ON o.id_ocorrencia_softlog = nfo.id_ocorrencia
		LEFT JOIN scr_ocorrencia_obs_edi obs	
			ON nf.id_ocorrencia_obs = obs.codigo_edi_obs 
		LEFT JOIN filial f 
