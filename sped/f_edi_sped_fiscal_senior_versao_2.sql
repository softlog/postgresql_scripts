	...
	*/	
	...
	,f AS (
		SELECT 	
			...
			parametros.valor_parametro::text as cod_cta_inventario,
			CASE WHEN cidades.uf NOT IN ('EX') THEN 2 ELSE NULL END::integer as tipo_imposto_filial
		FROM 	
			sped_fiscal,
			filial	
			LEFT JOIN cidades  
			...
			LEFT JOIN empresa 
				ON filial.codigo_empresa = empresa.codigo_empresa
			LEFT JOIN parametros
				ON filial.codigo_empresa = parametros.cod_empresa
	...
	c AS (
		SELECT 	
			...
			f_scr_get_cst(COALESCE(f.tipo_imposto_filial, c.tipo_imposto)) as cst,
			modal, 	
			id_conhecimento,	
			...
			CASE 	WHEN f_scr_get_cst(COALESCE(f.tipo_imposto_filial, c.tipo_imposto)) IN ('000')				
				THEN COALESCE(imposto,0.00)
				WHEN f_scr_get_cst(COALESCE(f.tipo_imposto_filial, c.tipo_imposto)) IN ('040','060')				
				THEN 0.00
				WHEN f_scr_get_cst(COALESCE(f.tipo_imposto_filial, c.tipo_imposto)) IN ('020','090')				
			...
			COALESCE(f.tipo_imposto_filial, c.tipo_imposto)::integer as tipo_imposto,     
...
-- 				id_produto_softlog
			UNION 
			SELECT 
				f.cod_item_1400_sped_fiscal as id_produto
			FROM 
				f
			WHERE
		...
		WITH temp AS (
			SELECT (row_to_json(row,true))::json as json FROM (
				SELECT 
					...
					''::text as cod_part,
					''::text as txt_compl,
					cod_cta_inventario::text as cod_cta,
		...
		WITH temp AS (
			SELECT (row_to_json(row,true))::json as json FROM (
				SELECT 
					...
					vl_icms as vl_icms,					
					--Estudar o caso de Substituicao Tributaria
					...
					''::text as chave_doc_e,
					''::text as fin_doc_e,
					''::text as chave_doc_e_ref,
					''::text as ind_dest,
					''::text as cod_mun_dest,
					''::text as cod_cta,
					reg_c590.reg_c590
				FROM
					contas_consumo
