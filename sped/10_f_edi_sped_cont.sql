...
BEGIN
	...
	/*	
 	sped_cont AS (
 		SELECT 
			'2020-06-01'::date as inicio,
			'2020-06-30'::date as fim,
	...
	e AS (
		...
		FROM 			
			...
			LEFT JOIN filial 
				ON filial.cnpj = empresa.cnpj
			LEFT JOIN sped_cta cta_empresa
				ON  cta_empresa.id = empresa.id_cta_transporte
		WHERE
	...
	f AS (
		SELECT 	
			...
			cta_filial.cod_cta as cod_cta,
			cta_filial.id as id_cta
		FROM 				
			e
			LEFT JOIN filial	
			...
			LEFT JOIN com_produtos 
				ON com_produtos.id_produto = filial.cod_item_1400_sped_fiscal
			LEFT JOIN sped_cta cta_filial 
				ON  cta_filial.id = com_produtos.id_cta
	...
	,pis_contas_consumo as (
		SELECT
			...
			MAX(ci.cod_cta) as cod_cta,
			MAX(ci.id_cta) as id_cta
	...
	,apur_imp AS (
		SELECT 
			ind,
			CASE WHEN c.destinatario_uf = 'EX' THEN '07' ELSE '01' END::character(2) as cst_pis,
			CASE WHEN c.destinatario_uf = 'EX' THEN 0.00 ELSE total_frete END::numeric(12,2) as vl_item_pis,
			CASE WHEN c.destinatario_uf = 'EX' THEN 0.00 ELSE c.total_frete END::numeric(12,2) as vl_bc_pis,
			CASE WHEN c.destinatario_uf = 'EX' THEN 0.00 ELSE i.aliq_pis END::numeric(12,2) as aliq_pis,
			CASE WHEN c.destinatario_uf = 'EX' THEN 0.00 ELSE (total_frete * (i.aliq_pis/100)) END::numeric(12,2) as vl_pis,
			CASE WHEN c.destinatario_uf = 'EX' THEN '07' ELSE '01' END::character(2) as cst_cofins,
			CASE WHEN c.destinatario_uf = 'EX' THEN 0.00 ELSE total_frete END::numeric(12,2) as vl_item_cofins,
			CASE WHEN c.destinatario_uf = 'EX' THEN 0.00 ELSE total_frete END::numeric(12,2) as vl_bc_cofins,
			CASE WHEN c.destinatario_uf = 'EX' THEN 0.00 ELSE i.aliq_cofins END::numeric(12,2) as aliq_cofins,
			CASE WHEN c.destinatario_uf = 'EX' THEN 0.00 ELSE (total_frete * (i.aliq_cofins/100)) END::numeric(12,2) as vl_cofins,
			c.cod_cta,
			c.id_cta
		FROM 
			i,
	...
	, cta AS (
		WITH t AS (
			...
			FROM 
				nfes_cred 
				LEFT JOIN compras_itens ci
					ON ci.id_compra = nfes_cred.id_compra
			GROUP BY 
				ci.id_cta
			UNION SELECT 
				ci.id_cta
			FROM 
				contas_cred
				LEFT JOIN compras_itens ci
					ON ci.id_compra = contas_cred.id_compra
			GROUP BY 
				ci.id_cta
			UNION SELECT 
				ci.id_cta
			FROM 
				telefone_cred
				LEFT JOIN compras_itens ci
					ON ci.id_compra = telefone_cred.id_compra
			GROUP BY 
				ci.id_cta
			UNION SELECT 
				si.id_cta
		...
		SELECT 
			t.id_cta,
			COALESCE(to_char(sped_cta.data_alteracao,'DDMMYYYY'),'') as data_alteracao,
			COALESCE(lpad(sped_cta.cod_nat_cc::text,2,'0'),'') as cod_nat_cc,
			COALESCE(sped_cta.ind_cta::text,'') as ind_cta,
			COALESCE(sped_cta.nivel::text,'') as nivel,
			COALESCE(trim(sped_cta.cod_cta),'') as cod_cta,
			COALESCE(trim(sped_cta.nome_cta),'') as nome_cta,
			COALESCE(trim(sped_cta.cod_cta_ref),'') as cod_cta_ref
		FROM 
			t
			LEFT JOIN sped_cta
				On t.id_cta = sped_cta.id			
	...
	,totais_bloco_0000 AS (
		WITH t AS (
			...
			SELECT 9, count(*) as qt FROM cta 		UNION
			SELECT 8, 3 as qt	   			
		)
		SELECT sum(qt) as total FROM t
	) 
	...
	,reg_0500 AS (		
		WITH temp AS (
			SELECT (row_to_json(row,true))::json as json FROM (
				SELECT 
					'0500' as reg,
					cta.data_alteracao as dt_alt,
					cta.cod_nat_cc,
					cta.ind_cta,
					cta.nivel::text as nivel,
					cta.cod_cta,
					cta.nome_cta,
					cta.cod_cta_ref,
					''::text as cnpj_est
		...
		SELECT array_agg(json) as reg_0500 from temp
	) 
