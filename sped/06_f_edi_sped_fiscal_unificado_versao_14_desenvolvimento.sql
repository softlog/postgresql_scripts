...
-- 	)
	SELECT 
 			'2020-01-01'::date as inicio,
 			'2020-01-31'::date as fim,
 			f_edi_sped_versao('2020-01-01'::date, 'EFD-ICMS-IPI') as versao,
 			'0'::text as cod_fin,
	...
	,nfe_itens AS (
		SELECT 
			...
			nfe.vl_mercadoria,
			nfe.id_natureza_operacao,
			i.issqn_valor_bc as vl_base_iss, 
			i.issqn_aliquota as aliquota_iss,
			(i.issqn_valor_bc * (i.issqn_aliquota/100))::numeric(12,2) as vl_iss,
			COALESCE(codigo_servico,1602) as codigo_servico
		FROM 
			nfe 
	...
	, nfe_totais_iss AS (
		SELECT 
			id_nf,								
			SUM(vl_total) as vl_total_servico,				
			SUM(vl_base_iss) as vl_base_iss, 
	...
	-------------------------------------------------------------------------------------------------
	--- Dados para o Bloco H - Inventario                                                          --
	-------------------------------------------------------------------------------------------------
			...
			SELECT (row_to_json(row,true))::json as json FROM (
				WITH t AS (
					...
					FROM
						nfe		
					WHERE
						nfe.id_natureza_operacao NOT IN ('5933','6933')
					UNION 
					SELECT 
						'C100' AS reg,
						ind_oper,
