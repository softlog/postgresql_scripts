...
SELECT id_romaneio, emitido FROM scr_romaneios WHERE numero_romaneio = '0020010089526';
	SELECT * FROM msg_fila_envio WHERE id_banco_dados = 123
		SELECT COUNT(*) 
		...
		WHERE 
			...
			AND msg_subscricao.id_fornecedor = 98
			AND msg_notificacao.ativo = 1;

			
UPDATE scr_romaneios SET emitido = 0 WHERE id_romaneio = 137212;
UPDATE scr_romaneios SET emitido = 1 WHERE id_romaneio = 137212;


UPDATE scr_romaneios SET emitido = 0 WHERE id_romaneio IN (524, 525);
UPDATE scr_romaneios SET emitido = 1 WHERE id_romaneio IN (524, 525);
...
BEGIN		
	...
	IF TG_TABLE_NAME = 'scr_romaneios' AND TG_OP = 'UPDATE' THEN 
		...
		RAISE NOTICE 'Nao tem notificacao 401';
		...
		ELSE
			RAISE NOTICE 'Nao tem notificacao 400';
		END IF;	
