BEGIN; 
		...
		--SELECT * FROM cidades WHERE id_cidade = 3231

		WITH 
		t AS (
			SELECT '{"numero_tabela_motorista":"0020010000552", 
				"id_origem":3231, 
				"id_destino":null, 
				"id_regiao_destino":280, 
				"diarias":1.0, 
				"km_rodados":201.00, 
				"total_pedagio":2.0000, 
				"placa_veiculo":"LSX6719", 
				"total_ajudantes":0, 
				"total_volume":1, 
				"total_peso":2.0000, 
				"total_cubagem":0.0000, 
				"total_servicos":1, 
				"total_frete":0.000000}'::json as vParametros
		),
		parametros AS (
			SELECT 
...
SELECT f_acerto_motorista_2(
	...
	"total_cubagem":0.0000, 
	"total_servicos":1, 
	"total_frete":0.000000}'::json,
'cursor_fechamento','msg') as resultado