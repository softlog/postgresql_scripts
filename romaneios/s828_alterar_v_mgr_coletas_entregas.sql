...
SELECT  scr_romaneios.id_romaneio,
        ...
        CASE
            ...
            WHEN scr_romaneios.problema_integracao = 1 THEN 'Pendência.Integração'::text
            WHEN scr_romaneios.emitido = 1 AND scr_romaneios.baixa = 0 THEN 'Emitido'::text
            WHEN scr_romaneios.emitido = 1 AND scr_romaneios.baixa = 0 AND scr_romaneios.viajando = 1 THEN 'Viajando'::text
        ...
        CASE
            ...
            ELSE NULL::text
        END::character(19)::text, ' / '::text) AS lst_entregas,
        count(col_coletas.id_coleta)::numeric AS qtcoletas,
        string_agg(col_coletas.id_coleta_filial::text, ' / '::text) AS lst_coletas,
        ...
        COALESCE(scr_romaneios.id_setor,0)::INTEGER AS id_setor,
        COALESCE(sd.descricao,'')::CHAR(35) AS setor_desc,
        COALESCE(scr_romaneios.id_regiao,0)::INTEGER AS id_regiao,
        COALESCE(regiao.descricao,'')::CHAR(35) AS regiao_desc
...
FROM    scr_romaneios
        ...
        LEFT JOIN v_setor_destino sd ON scr_romaneios.id_setor = sd.id_regiao
        LEFT JOIN regiao ON scr_romaneios.id_regiao = regiao.id_regiao
WHERE   scr_romaneios.tipo_romaneio = 1 
AND     scr_romaneios.numero_romaneio IS NOT NULL

GROUP BY scr_romaneios.id_romaneio, 
        scr_relatorio_viagem.id_relatorio_viagem, 
        fornecedores.id_fornecedor, 
        origem.id_cidade, 
        destino.id_cidade, 
        redespachador.id_fornecedor,
        sd.descricao,regiao.descricao;
