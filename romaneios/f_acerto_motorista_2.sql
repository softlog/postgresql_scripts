	...
	WITH 
		parametros AS (
			SELECT 
				...
				COALESCE(((vParametros->>'total_peso')::text)::numeric(12,2),
				((vParametros->>'peso')::text)::numeric(12,2)) as total_peso,
				((vParametros->>'total_pedagio')::text)::numeric(12,2) as peso_pedagio,
				COALESCE(((vParametros->>'total_cubagem')::text)::numeric(12,2),
				((vParametros->>'cubagem')::text)::numeric(12,2)) as total_cubagem,
				...
				((vParametros->>'id_destinatario')::text)::integer as id_destinatario,
				1::integer as qt_viagens,
				...
				COALESCE(
				((vParametros->>'id_regiao_destino')::text)::integer, 
				...
				(vParametros->>'placa_veiculo')::text as placa_veiculo
		...
		, destinatario AS (
			...
			FROM 
				parametros ent 
				LEFT JOIN cliente 
					ON cliente.codigo_cliente = ent.id_destinatario
		...
		, regras_calculo AS ( 
			SELECT 	
				...
				CASE 	WHEN tipo_calculo = 1	THEN total_peso - (total_peso * COALESCE(percentual_reducao,0)/100)
					WHEN tipo_calculo = 2	THEN total_cubagem - (total_cubagem * COALESCE(percentual_reducao,0)/100)
					...
					WHEN tipo_calculo IN (11,18) THEN total_frete - (total_frete * COALESCE(percentual_reducao,0)/100)
				...
				valor_fixo_excedido,
				(percentual_reducao/100) as percentual_reducao
			FROM 
				parametros
			...
			WHERE 
				...
				AND (
					CASE 	WHEN scr_tabela_motorista_regioes.placa_veiculo IS NOT NULL 
						THEN parametros.placa_veiculo = scr_tabela_motorista_regioes.placa_veiculo
						ELSE true
					END
				)
				AND (
				...
				AND -- Filtrar Faixa de Valores de Acordo com o tipo de calculo... 
					CASE WHEN medida_final = 0 THEN -- Se medida final = 0 então postula-se um valor máximo inatingível...
									CASE 	WHEN tipo_calculo = 1 THEN total_peso >= medida_inicial AND total_peso <= 999999999 
										...
										WHEN tipo_calculo IN (11,18) THEN total_frete >= medida_inicial AND total_frete <= 999999999 									
								   ...
								   ELSE 
									CASE 	WHEN tipo_calculo = 1 THEN total_peso >= medida_inicial 
										...
										WHEN tipo_calculo IN (11,18) THEN total_frete >= medida_inicial 
										...
										--WHEN qt_viagens THEN qt_viagens >= medida_inicial 
		...
		valores_calculo AS (
			...
			FROM regras_calculo 
			ORDER BY id_regiao_origem DESC LIMIT 1
		),
		resultado_final AS (
			SELECT 			
				id_romaneio,
...
/*
ALTER  FUNCTION public.f_acerto_motorista_2(
    vparametros json,
    cfechamento refcursor,
    msg refcursor) OWNER TO softlog_saocarlos2
   */
