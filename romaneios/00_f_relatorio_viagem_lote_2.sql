...
CREATE OR REPLACE FUNCTION f_relatorio_viagem_automatico_2(
			p_cod_empresa text, 
			p_cod_filial text, 
			p_chave text, 
			por_veiculo integer, 
			...
			p_data_inicio date, 
			p_data_fim date, 
			p_valor_pagar numeric(12,2), 
			qt_diarias integer, 
			p_lista_romaneios text)
  RETURNS integer AS
$BODY$
DECLARE
     qt integer;          
...
BEGIN
	...
	str_sql = 'UPDATE scr_romaneios SET id_acerto = ' || v_id_relatorio_viagem::text 
	...
	WITH t AS ( 
		...
		FROM 
			...
			LEFT JOIN v_scr_romaneio_despesas 
				ON  r.id_romaneio = v_scr_romaneio_despesas.id_romaneio
		...
		WHERE 
			r.id_acerto = v_id_relatorio_viagem
